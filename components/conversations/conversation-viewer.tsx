...
import { ChevronLeft, ChevronRight, Quote, Infinity } from "lucide-react";
...
export function ConversationViewer({ messages: moments }: ConversationViewerProps) {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto pt-4 pb-36 md:py-8 px-4 md:px-8 relative isolate">
            ...
            {/* --- CONTENT AREA --- */}
            <div className="relative mt-20 min-h-[500px] md:min-h-[600px] perspective-1000">
                <AnimatePresence initial={false} custom={direction} mode="wait">
                    <motion.div
                        ...
                        animate="center"
                        exit="exit"
                        className="absolute w-full h-full flex items-center justify-center p-0 md:p-2"
                    >
                        {/* 3D TILT WRAPPER REMOVED AS PER REQUEST */}

                        <Card className="bg-white/60 dark:bg-stone-950/40 backdrop-blur-xl border border-white/40 dark:border-white/10 p-4 md:p-10 shadow-2xl shadow-stone-200/50 dark:shadow-black/50 rounded-[2rem] min-h-[450px] md:min-h-[500px] flex flex-col justify-center relative overflow-hidden w-full max-w-3xl">
                            ...
                            <div className="absolute top-6 right-6 md:top-8 md:right-10 opacity-10">
                                ...
                                {currentMoment.map((msg, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="relative group shrink-0">
                                            <div className={`absolute -inset-0.5 rounded-full blur opacity-40 group-hover:opacity-75 transition duration-500 ${getUserColor(msg.name || "Unknown").replace('bg-', 'bg-gradient-to-r from-transparent to-')}`}></div>
                                            <Avatar className={`w-8 h-8 md:w-12 md:h-12 border-2 border-white dark:border-stone-800 shadow-md relative z-10 ${getUserColor(msg.name || "Unknown")}`}>
                                                <AvatarFallback className="text-white text-[9px] md:text-[11px] font-bold tracking-wider w-full h-full flex justify-center items-center bg-transparent">
                                                    {getInitials(msg.name || "Unknown Friend")}
                                                </AvatarFallback>
                                            </Avatar>
                                        </div>
                                        ...
                                        <div className={`max-w-[85%] space-y-1.5 md:space-y-2 ${idx % 2 === 0 ? 'items-start' : 'items-end flex flex-col'}`}>
                                            <div className={`flex flex-col px-1 ${idx % 2 === 0 ? 'items-start' : 'items-end'}`}>
                                                ...
                                                <span className="text-xs md:text-sm font-bold text-stone-700 dark:text-stone-200 tracking-tight">{msg.name || "Unknown Friend"}</span>
                                            </div>
